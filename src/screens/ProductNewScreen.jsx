...
function NewProductScreen() {
  ...
  const { loading, error, success } = useSelector((state) => state.productAdd);
  ...
  const uploadFileHandler = async (e) => {
    ...
    try {
      const data = await uploadImage(formData, userInfo.token);
      setImage(data);
      setUploading(false);
    } catch (error) {
      setUploading(false);
      ...
      <FormContainer>
        ...
        <Form onSubmit={submitHandler}>
          ...
          <Form.Group controlId="image">
            ...
            {uploading && <Loader />}
            {uploadError && (
              <Message className="mt-2" variant="danger">
                {uploadError}
              </Message>
            )}
          </Form.Group>
